...
class SecureStorage:
    ...
    def __init__(self, storage_dir: Optional[Path] = None):
        self.storage_dir = storage_dir or Path.home() / ".skillforge"
    ...
    def verify_admin(self, username: str, password: str) -> bool:
        """Verify admin credentials (constant-time comparison)."""
        ...
        username_ok = hmac.compare_digest(
            self._data.get('_admin_username', '').encode(), username.encode()
        )
        hash_ok = hmac.compare_digest(pw_hash, stored_hash)
        return username_ok and hash_ok
