...
class PatternDetector:
    ...
    def get_auto_skill_prompt(self, user_id: str) -> Optional[str]:
        """
        Build a system prompt injection for autonomous skill creation.

        When high-confidence patterns are detected (>=0.85, >=5 occurrences),
        instructs the LLM to proactively create a skill to automate the pattern.
        Only triggers for the single highest-confidence pattern that hasn't
        already been converted or dismissed.

        Args:
            user_id: User identifier

        Returns:
            Prompt string to inject, or None if no patterns warrant auto-creation
        """
        # Find patterns eligible for auto-creation
        # Higher threshold than suggestions — we're acting without asking
        ...
        candidates = [
            p for p in self._patterns[user_id]
            if not p.dismissed
            and not p.created_skill
            and p.confidence >= AUTO_CONFIDENCE
            and p.occurrences >= AUTO_OCCURRENCES
        ]

        if not candidates:
            return None

        # Pick the single highest-confidence pattern
        candidates.sort(key=lambda p: (p.confidence, p.occurrences), reverse=True)
        pattern = candidates[0]

        examples = ", ".join(f'"{e}"' for e in pattern.example_commands[:3])
        skill_name = pattern.suggested_skill_name or "auto-task"

        prompt = (
            f"\n\n**[Auto-Skill Creation]** "
            f"You have noticed the user repeatedly performs this action: "
            f"{pattern.description} (seen {pattern.occurrences} times, "
            f"examples: {examples}). "
            f"Create a skill to automate this for them. Emit a ```create-skill``` "
            f"code block with ACTION: create, NAME: {skill_name}, a clear "
            f"DESCRIPTION, and detailed INSTRUCTIONS that capture what the user "
            f"typically does. Briefly tell the user you noticed the pattern and "
            f"created the skill for them. "
