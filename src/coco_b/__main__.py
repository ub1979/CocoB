#!/usr/bin/env python3
"""
coco B — AI assistant with persistent memory.
...
Usage:
    ...
    coco-b doctor      Check config, dependencies, and connections
"""
import sys
...
def _run_doctor():
    ...
    core_modules = [
        ("coco_b.core.router", "MessageRouter"),
        ("coco_b.core.sessions", "SessionManager"),
        ("coco_b.core.personality", "PersonalityManager"),
        ("coco_b.core.scheduler", "SchedulerManager"),
        ("coco_b.core.memory.sqlite_memory", "SQLiteMemory"),
    ]
...
def main():
    ...
    if cmd == "ui":
        import flet as ft
        from coco_b.flet.app import main as app_main
        ft.app(target=app_main)

    elif cmd == "gradio":
        import runpy
        runpy.run_module("coco_b.gradio_ui", run_name="__main__")

    elif cmd == "bot":
        from coco_b.bot import main as bot_main
        bot_main()

    elif cmd == "telegram":
        import runpy
        runpy.run_module("coco_b.telegram_bot", run_name="__main__")

    elif cmd == "slack":
        import runpy
        runpy.run_module("coco_b.run_slack", run_name="__main__")

    elif cmd == "discord":
        import runpy
        runpy.run_module("coco_b.run_discord", run_name="__main__")
