...
from coco_b import PROJECT_ROOT as project_root
from coco_b.flet.theme import (
...
try:
    from coco_b.core.scheduler import SchedulerManager, ScheduledTask
...
class SettingsView:
    ...
    def build(self) -> ft.Column:
        self.settings_header = ft.Container(
            content=ft.Text("Settings", size=24, weight=ft.FontWeight.W_700, color=AppColors.PRIMARY),
            padding=ft.padding.only(bottom=16),
            border=ft.border.only(bottom=ft.BorderSide(1, AppColors.BORDER))
        )

        self.settings_scroll = ft.ListView(controls=[
            ...
            ft.Container(height=16),
            self._create_proactive_tasks_group(),
            ft.Container(height=16),
            self._create_llm_providers_group(),
            ft.Container(height=16),
            self._create_memory_section(),
        ], expand=True, spacing=0)

        return ft.Column([self.settings_header, self.settings_scroll], expand=True, spacing=0)
    ...
    def _create_appearance_section(self):
        self.dark_mode_switch = ft.Switch(
            value=AppColors.is_dark_mode(), active_color=AppColors.PRIMARY,
            on_change=self._toggle_dark_mode,
        )
        content = ft.Column([
            ft.Row([
                ft.Icon(icons.DARK_MODE, color=AppColors.TEXT_SECONDARY, size=24),
                ft.Column([
                    ft.Text("Dark Mode", size=14, weight=ft.FontWeight.W_600, color=AppColors.TEXT_PRIMARY),
                    ft.Text("Switch to dark theme for reduced eye strain", size=11, color=AppColors.TEXT_SECONDARY),
                ], spacing=2, expand=True),
                self.dark_mode_switch,
            ], spacing=12),
        ], spacing=0)
        return CollapsibleSection(title="Appearance", icon=icons.PALETTE, content=content, expanded=True)
    ...
    def _create_personas_section(self):
        ...
        content = ft.Column([
            ft.Text("Loaded Personas", size=13, weight=ft.FontWeight.W_600, color=AppColors.TEXT_PRIMARY),
            self._persona_list,
            ft.Divider(height=1, color=AppColors.BORDER),
            ft.Text("Channel Defaults", size=13, weight=ft.FontWeight.W_600, color=AppColors.TEXT_PRIMARY),
            ft.Text("Set a default persona for each channel:", size=11, color=AppColors.TEXT_SECONDARY),
            ft.Row(channel_row_items, wrap=True, spacing=8),
            ft.Divider(height=1, color=AppColors.BORDER),
            ft.Text("Create New Persona", size=13, weight=ft.FontWeight.W_600, color=AppColors.TEXT_PRIMARY),
            ft.Row([self._persona_create_name, self._persona_create_emoji, self._persona_create_desc], spacing=8),
            self._persona_create_instructions,
            ft.ElevatedButton("Create Persona", icon=icons.ADD, on_click=self._on_create_persona),
        ], spacing=10)

        return CollapsibleSection(title="Personas & Agents", icon=icons.PERSON, content=content, expanded=False)
    ...
    def _create_messaging_bots_group(self):
        ...
        def create_sub_item(title, icon_emoji, status, status_color, content, expanded=False):
            is_expanded = expanded
            expand_icon = ft.Icon(icons.EXPAND_LESS if is_expanded else icons.EXPAND_MORE, color=AppColors.TEXT_SECONDARY, size=18)
            content_container = ft.Container(content=content, visible=is_expanded, padding=ft.padding.only(left=16, top=8))

            def toggle(e):
                nonlocal is_expanded
                is_expanded = not is_expanded
                content_container.visible = is_expanded
                expand_icon.name = icons.EXPAND_LESS if is_expanded else icons.EXPAND_MORE
                self.page.update()

            header = ft.Container(
                content=ft.Row([
                    ft.Text(icon_emoji, size=18),
                    ft.Text(title, size=13, weight=ft.FontWeight.W_500, color=AppColors.TEXT_PRIMARY, expand=True),
                    ft.Container(
                        content=ft.Text(status, size=10, color="white", weight=ft.FontWeight.W_600),
                        bgcolor=status_color, padding=ft.padding.symmetric(horizontal=8, vertical=2),
                        border_radius=ft.border_radius.all(10),
                    ),
                    expand_icon,
                ], spacing=10),
                padding=ft.padding.symmetric(horizontal=12, vertical=10),
                bgcolor=AppColors.SURFACE_VARIANT, border_radius=ft.border_radius.all(8), on_click=toggle,
            )
            return ft.Container(content=ft.Column([header, content_container], spacing=0), margin=ft.margin.only(bottom=8))

        group_content = ft.Column([
            create_sub_item("Telegram", "\U0001f4f1", "Ready", AppColors.SUCCESS, telegram_content),
            create_sub_item("WhatsApp", "\U0001f4ac", "Ready", AppColors.SUCCESS, whatsapp_content),
            create_sub_item("Slack", "\U0001f4bc", "Ready", AppColors.SUCCESS, slack_content),
            create_sub_item("Discord", "\U0001f3ae", "Coming Soon", "#eab308", discord_content),
        ], spacing=0)

        return CollapsibleSection(title="Messaging Bots", icon=icons.FORUM, content=group_content, expanded=True)
    ...
    def _create_telegram_content(self):
        if not hasattr(self, 'telegram_token_input'):
            self._create_telegram_section()
        return ft.Column([
            ft.Text("Token is encrypted locally. Set a password to protect viewing it.", size=12, color=AppColors.TEXT_SECONDARY),
            ft.Container(height=12),
            ft.Row([self.telegram_token_input, self.telegram_reveal_btn], spacing=8),
            ft.Container(height=12),
            ft.Row([self.telegram_status_icon, self.telegram_status_text, ft.Container(expand=True), self.telegram_start_btn], spacing=8),
            ft.Container(height=8),
            ft.Row([self.telegram_bot_info, ft.Container(expand=True), self.telegram_auto_start], spacing=8),
    ...
    def _show_password_dialog(self, title, message, on_success):
        ...
        dlg = ft.AlertDialog(
            ...
            actions=[ft.TextButton("Cancel", on_click=lambda e: self.page.close(dlg)), ft.ElevatedButton("OK", on_click=on_submit)],
        )
        self.page.open(dlg)
    ...
    def _show_set_password_dialog(self, on_success):
        ...
        dlg = ft.AlertDialog(
            ...
            actions=[ft.TextButton("Cancel", on_click=lambda e: self.page.close(dlg)), ft.ElevatedButton("Set Password", on_click=on_submit)],
        )
        self.page.open(dlg)
    ...
    def _create_whatsapp_section(self):
        ...
        self.whatsapp_qr_container = ft.Container(
            ...
            bgcolor=AppColors.SURFACE_VARIANT, border_radius=ft.border_radius.all(8), padding=ft.padding.all(20), visible=False,
        )
    ...
    def _create_whatsapp_content(self):
        if not hasattr(self, 'whatsapp_service_url'):
            self._create_whatsapp_section()
        return ft.Column([
            ft.Text("Connect to WhatsApp via Baileys Node.js service.", size=12, color=AppColors.TEXT_SECONDARY),
            ft.Container(height=12),
            self.whatsapp_service_status, ft.Container(height=4),
            self.whatsapp_connection_status, ft.Container(height=4),
            self.whatsapp_bot_status, ft.Container(height=12),
            ft.Text("1. Service Control:", size=11, weight=ft.FontWeight.W_600, color=AppColors.TEXT_PRIMARY),
            ft.Container(height=8), self.whatsapp_start_service_btn,
            ft.Container(height=8), self.whatsapp_stop_service_btn,
            ft.Container(height=8), self.whatsapp_check_btn, ft.Container(height=12),
            ft.Text("2. Connect WhatsApp:", size=11, weight=ft.FontWeight.W_600, color=AppColors.TEXT_PRIMARY),
            ft.Container(height=8), self.whatsapp_show_qr_btn,
            ft.Container(height=8), self.whatsapp_disconnect_btn,
            ft.Container(height=8), self.whatsapp_logout_btn,
            ft.Container(height=12), self.whatsapp_qr_container, ft.Container(height=12),
            ft.Text("3. Start Bot:", size=11, weight=ft.FontWeight.W_600, color=AppColors.TEXT_PRIMARY),
            ft.Container(height=8), self.whatsapp_start_bot_btn,
            ft.Container(height=8), self.whatsapp_stop_bot_btn,
            ft.Container(height=8), self.whatsapp_auto_start, ft.Container(height=16),
            ft.Text("4. Access Control:", size=11, weight=ft.FontWeight.W_600, color=AppColors.TEXT_PRIMARY),
            ft.Container(height=8), self.whatsapp_dm_policy,
            ft.Container(height=8), self.whatsapp_allowlist,
            ft.Container(height=8), self.whatsapp_group_policy,
            ft.Container(height=8), self.whatsapp_ack_reaction, ft.Container(height=16),
            ft.Text("5. Contact Personas:", size=11, weight=ft.FontWeight.W_600, color=AppColors.TEXT_PRIMARY),
            ft.Container(height=8), self.whatsapp_persona_list, ft.Container(height=8),
            ft.Row([self.whatsapp_persona_phone_input, self.whatsapp_persona_dropdown, self.whatsapp_persona_add_btn], spacing=8),
            ft.Container(height=12), ft.Text("Service URL:", size=11, color=AppColors.TEXT_MUTED),
            ft.Container(height=4), self.whatsapp_service_url,
        ], spacing=0)
    ...
    def _create_slack_content(self):
        if not hasattr(self, 'slack_bot_token_input'):
            self._create_slack_section()
        return ft.Column([
            ft.Text("Connect via Socket Mode. Get tokens from api.slack.com/apps", size=12, color=AppColors.TEXT_SECONDARY),
            ft.Container(height=12),
            ft.Row([self.slack_bot_token_input, self.slack_reveal_btn], spacing=8),
            ft.Container(height=8), self.slack_app_token_input,
            ft.Container(height=12),
            ft.Row([self.slack_status_icon, self.slack_status_text, ft.Container(expand=True), self.slack_start_btn], spacing=8),
            ft.Container(height=8),
            ft.Row([self.slack_bot_info, ft.Container(expand=True), self.slack_auto_start], spacing=8),
        ], spacing=0)
    ...
    def _create_llm_providers_group(self):
        ...
        def create_sub_item(title, icon_emoji, description, content, expanded=False):
            is_expanded = expanded
            expand_icon = ft.Icon(icons.EXPAND_LESS if is_expanded else icons.EXPAND_MORE, color=AppColors.TEXT_SECONDARY, size=18)
            content_container = ft.Container(content=content, visible=is_expanded, padding=ft.padding.only(left=16, top=8))
        ...
        group_content = ft.Column([
            create_sub_item("Local LLM Servers", "\U0001f5a5\ufe0f", "Ollama, LM Studio, MLX, vLLM", local_content),
            create_sub_item("CLI Providers", "\u2328\ufe0f", "Claude CLI, Gemini CLI (subscription)", cli_content),
            create_sub_item("Cloud API", "\u2601\ufe0f", "OpenAI, Anthropic, Groq, etc.", cloud_content),
        ], spacing=0)

        return CollapsibleSection(title="LLM Providers", icon=icons.PSYCHOLOGY, content=group_content, expanded=True)
    ...
    def _create_local_servers_content(self):
        ...
        return ft.Column([
            ft.Text("Local LLM servers that run on your machine.", size=12, color=AppColors.TEXT_SECONDARY),
            ft.Container(height=12), self._local_server_list,
            ft.Container(height=16),
            ft.Row([StyledButton("Refresh Status", icon=icons.REFRESH, on_click=self._refresh_local_servers, variant="secondary")]),
            ft.Divider(color=AppColors.BORDER),
            ft.Text("Select Local Provider", size=14, weight=ft.FontWeight.W_600, color=AppColors.TEXT_PRIMARY),
            ft.Container(height=12), self._local_provider_btn_row,
            ft.Container(height=16), self.model_status_text,
            ft.Container(height=8), self.local_model_dropdown,
            ft.Container(height=12),
            ft.Row([
                StyledButton("Refresh Models", icon=icons.REFRESH, on_click=self._refresh_local_models, variant="secondary"),
                StyledButton("Use Local Server", icon=icons.PLAY_ARROW, on_click=self._use_local_server),
            ], spacing=12),
        ], spacing=0)
    ...
    def _create_local_servers_section(self):
        ...
        for name in LOCAL_SERVER_PROVIDERS.keys():
            ...
            btn = ft.ElevatedButton(
                content=ft.Row([ft.Text(icon_text, size=18), ft.Text(name.upper(), weight=ft.FontWeight.W_600)], spacing=8, alignment=ft.MainAxisAlignment.CENTER),
                bgcolor=AppColors.SURFACE_VARIANT, color=AppColors.TEXT_PRIMARY,
                style=ft.ButtonStyle(padding=ft.padding.symmetric(horizontal=20, vertical=14), shape=ft.RoundedRectangleBorder(radius=12), side=ft.BorderSide(2, AppColors.BORDER)),
    ...
    def _create_cli_providers_content(self):
        ...
        return ft.Column([
            ft.Text("CLI providers use your subscription (no API key needed).", size=12, color=AppColors.TEXT_SECONDARY),
            ft.Container(height=12), self._cli_list,
            ft.Container(height=12),
            ft.Row([StyledButton("Refresh Status", icon=icons.REFRESH, on_click=self._refresh_cli_status)]),
            ft.Container(height=8), self.cli_status_text,
        ], spacing=0)
    ...
    def _create_cloud_api_content(self):
        ...
        return ft.Column([
            ft.Text("Cloud API providers require an API key.", size=12, color=AppColors.TEXT_SECONDARY),
            ft.Container(height=12), self.cloud_provider_dropdown,
            ft.Container(height=12),
            ft.Row([StyledButton("Switch Provider", icon=icons.CLOUD, on_click=self._use_cloud_provider)]),
            ft.Container(height=8), self.cloud_status_text,
            ft.Divider(color=AppColors.BORDER),
            ft.Text("API Key Management", size=14, weight=ft.FontWeight.W_600, color=AppColors.TEXT_PRIMARY),
            ft.Row([self.api_key_provider, self.api_key_input], spacing=12),
            ft.Container(height=8),
            ft.Row([
                StyledButton("Check Key", icon=icons.SEARCH, on_click=self._check_api_key_status, variant="secondary"),
                StyledButton("Save Key", icon=icons.SAVE, on_click=self._save_api_key),
            ], spacing=12),
            ft.Container(height=8), self.api_key_status,
        ], spacing=0)
    ...
    def _create_memory_section(self):
        content = ft.Column([
            ft.Switch(label="Enable Persistent Memory", value=True),
            ft.Slider(label="Max Context Messages", min=5, max=50, divisions=45, value=20),
            ft.Slider(label="Memory Importance Threshold", min=0, max=1, divisions=10, value=0.5),
            StyledButton("Clear All Memory", icon=icons.DELETE, variant="outline"),
        ], spacing=16)
        return CollapsibleSection(title="Memory Settings", icon=icons.MEMORY, content=content, expanded=False)
    ...
    def _create_proactive_tasks_group(self):
        ...
        def create_sub_item(title, icon_emoji, description, content, expanded=False):
            ...
            def toggle(e):
                ...
                self.page.update()
            header = ft.Container(
                content=ft.Row([ft.Text(icon_emoji, size=18),
                    ft.Column([ft.Text(title, size=13, weight=ft.FontWeight.W_500, color=AppColors.TEXT_PRIMARY),
                               ft.Text(description, size=10, color=AppColors.TEXT_SECONDARY)], spacing=0, expand=True),
                    expand_icon], spacing=10),
                padding=ft.padding.symmetric(horizontal=12, vertical=10),
                bgcolor=AppColors.SURFACE_VARIANT, border_radius=ft.border_radius.all(8), on_click=toggle,
            )
            return ft.Container(content=ft.Column([header, content_container], spacing=0), margin=ft.margin.only(bottom=8))
        ...
        group_content = ft.Column([
            create_sub_item("Task List", "\U0001f4cb", f"{task_count} tasks configured", ft.Column([
                self._scheduler_task_list,
                ft.Row([StyledButton("Refresh", icon=icons.REFRESH, variant="outline", on_click=lambda e: self._scheduler_refresh_tasks())], spacing=8),
            ], spacing=8), expanded=True),
            create_sub_item("Create / Edit Task", "\u2795", "Add a new scheduled task", form_content),
            create_sub_item("Execution Log", "\U0001f4ca", "Recent task execution history", ft.Column([
                self._scheduler_log_list,
                ft.Row([StyledButton("Refresh Log", icon=icons.REFRESH, variant="outline", on_click=lambda e: self._scheduler_refresh_log())], spacing=8),
            ], spacing=8)),
        ], spacing=0)

        return CollapsibleSection(title="Proactive Tasks", icon=icons.SCHEDULE, content=group_content, expanded=False)
            ...
            card = ft.Container(
                content=ft.Row([
                    ...
                    ft.Container(content=ft.Text(status.upper(), size=9, color="white", weight=ft.FontWeight.W_600),
                                bgcolor=status_color, padding=ft.padding.symmetric(horizontal=6, vertical=2), border_radius=ft.border_radius.all(8)),
                ...
                bgcolor=AppColors.SURFACE_VARIANT, border_radius=ft.border_radius.all(8),
            )
