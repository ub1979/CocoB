...
# full SkillForgeApp without crashing.  Uses a MagicMock page so no Flet runtime
# is needed.
# =============================================================================
...
class TestComponentBuilds:
    ...
    def test_chat_message_assistant_markdown(self):
        ...
        bubble = msg.controls[1]
        assert isinstance(bubble, ft.Container)
        message_col = bubble.content
    ...
    def test_collapsible_section(self):
        from skillforge.flet.components.widgets import CollapsibleSection
        section = CollapsibleSection(
            title="Test", content=ft.Text("body"), expanded=True)
        assert isinstance(section, ft.Container)

    def test_server_status_card(self):
        from skillforge.flet.components.cards import ServerStatusCard
    ...
    def test_cli_status_card(self):
        from skillforge.flet.components.cards import CliStatusCard
...
class TestSkillForgeAppSmoke:
    """SkillForgeApp must initialise and build the full UI without errors."""

    def test_app_initialises(self, mock_page):
        """The full SkillForgeApp init — page setup, views, nav, layout — must not crash."""
        from skillforge.flet.app import SkillForgeApp
        app = SkillForgeApp(mock_page)

        # Page was configured
        assert mock_page.title == "SkillForge - Intelligent AI Assistant"
        ...
        # Nav rail created (5 destinations: Chat, Tools, Settings, History, Admin)
        assert app.nav_rail is not None
        assert len(app.nav_rail.destinations) == 5
    ...
    def test_main_entry_point(self, mock_page):
        ...
        Note: main() now shows a LoginView first; window_prevent_close
        is only set after successful authentication.
        """
        from skillforge.flet.app import main
        main(mock_page)
